...
df=pd.read_csv("Z:\Project\Streamlit\cars24-car-price.csv")
#st.dataframe(df)
...
plot=sns.pairplot(y_vars=['selling_price'],x_vars=[var1], data=df, height=8)
col1.pyplot(plot)
...
fig = plt.figure()
sns.histplot(x = var2, data = df, bins=bins)
col2.pyplot(fig)
